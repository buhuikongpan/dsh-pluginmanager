...
function scanToolNames(name) {
	...
	try {
		...
		if (dir !== null) {
			...
			// Per the cordis-plugin-development skill, a tool plugin is one that
			// registers model tools through the harness toolset (dynamic) or the
			// tools service (installed): harness.defineTool / harness.registerTool
			// / ctx.tools.register. Ordinary plugins use slots.register,
			// ctx.provide, ctx.on — those must NOT count as tool traces.
			const hasTrace = /(?:harness\.defineTool|harness\.registerTool|\.defineTool\s*\(|\.registerTool\s*\(|(?:ctx\.)?tools\.register\s*\()/.test(src);
			const tools = new Set();
