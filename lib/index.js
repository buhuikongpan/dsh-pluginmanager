...
// ── hot-mount remnants (dsh-market hot.ts context, isolated to plugin-manager/hot) ──
//
// The hot-mount activator (register → hotMount) was removed together with the
// "补登记" feature. The hooks below survive only to keep uninstall() honest:
// `hotUnmount`/`listHotMounts` release any manager-owned hot mount, and
// `cleanHotDir` wipes that input directory at boot. With no live mounts these
// are always no-ops, but uninstall still calls them so the bookkeeping stays
// correct if a mount is ever re-introduced.
...
class PluginManagerGateway extends TypertRemoteService {
	...
	constructor(ctx) {
		...
		cleanHotDir();
		for (const name of ["snapshot", "setEnabled", "uninstall", "saveDescription"]) {
	...
	async setEnabled(input) {
		...
		try {
			...
			if (!row.entryId) return { ok: false, name, error: "该插件没有可管理的激活行，无法切换（可先在配置文件里添加激活行）" };
...
function readPkgDshLocal(name) {
	const dir = packageDir(name);
	if (dir === null) return null;
	try {
		const manifest = readJson(join(dir, "package.json"));
		const dsh = manifest.dsh;
		return dsh && typeof dsh === "object" ? dsh : {};
	} catch {
		return null;
	}
}
...
/**
 * Plain-language "why is this plugin not effective" for one USER extension row,
 * plus the concrete fix steps and a copy-paste AI-repair prompt (dsh-market's
 * `reasons` / aiFix pattern). Returns null for rows that need no diagnosis
 * (native infrastructure, or a plugin that is live right now).
 *
 * Only user extensions are diagnosed — we never second-guess the native layers.
 */
function diagnoseRow({ native, name, source, phase, enabled, active }) {
	if (native || active) return null; // live / infrastructure: nothing to say
	let state;
	let reasons;
	let fixSteps;
	if (phase === "failed") {
		state = "failed";
		reasons = ["插件启动时加载失败（loader 相位 = failed）——通常是启动抛异常、依赖缺失或入口产物不存在"];
		fixSteps = [
			"查看服务端日志里这条插件的启动报错堆栈（dsh web 的终端 / 日志）",
			"核对入口产物是否存在：package.json 的 main 或 dsh.bundle 指向的文件",
			"到 profile 目录执行 pnpm install，补齐可能缺失的依赖",
			"检查它自带 cordis.patch.yml 激活行的 insert 目标是否正确"
		];
	} else if (phase === "pending" || phase === "loading") {
		state = "pending";
		reasons = ["插件正在启动（loader 相位 = " + phase + "），尚未完全生效"];
		fixSteps = ["稍等片刻；若长时间停在启动中，查看服务端日志确认是否卡住"];
	} else if (!enabled) {
		state = "disabled";
		reasons = ["已被停用（补丁层 disabled 或当前预设停用），所以当前未生效"];
		fixSteps = ["点「启用」恢复运行；若该行由预设控制，到对应预设里启用"];
	} else {
		const dsh = readPkgDshLocal(name);
		const isPluginPkg = dsh !== null && (dsh.bundle !== undefined || dsh.client !== undefined);
		if (isPluginPkg && (source === "dependency" || source === "other")) {
			state = "notActivated";
			reasons = ["它是 DSH 插件包，但未被激活为插件：既不在 profile 的 dsh.profile.bundles，也没有加载条目（patch 行）"];
			fixSteps = [
				"在 profile 的 package.json → dsh.profile.bundles 数组里加入它的包名，或",
				"在 cordis.patch.yml 加一条激活行（- id: " + name + "），保存后重启服务生效",
				"如果它只是被其它插件按名加载的库，可忽略本提示"
			];
		} else {
			state = "inactive";
			reasons = ["已配置但当前未激活（未运行或尚未加载）"];
			fixSteps = [
				"点「启用」尝试启动；仍无效则查看服务端日志确认原因",
				"确认入口产物与依赖齐全（可在 profile 目录 pnpm install）"
			];
		}
	}
	const aiFixPrompt = [
		"请帮我修复 DeepSeek Harness 的一个插件问题。profile 目录：" + profileDir(),
		"插件 " + name + "（来源：" + source + "）当前未生效。",
		"诊断：" + reasons.join("；"),
		"建议按这些步骤排查：" + fixSteps.join("；"),
		"请先检查 profile 的 package.json（dsh.profile.bundles）与 cordis.patch.yml，说明你的计划后再动手；只处理明确问题，每步说明理由并等待确认。"
	].join("\n");
	return { state, reasons, fixSteps, aiFixPrompt };
}
...
function buildRow({ entryId, name, state, nativeNames, layers, userRows, otherBundles, bundles, deps, descriptions, presetMap, presetInfo }) {
	...
	const active = presetControlled ? enabled : state?.phase === "active";
	return {
		entryId,
		name,
		...
		// Plain-language "why is this not effective" + fix steps + AI-repair
		// prompt, dsh-market style. `null` when the row needs no diagnosis.
		health: diagnoseRow({ native, name, source, phase: state?.phase ?? null, enabled, active }),
