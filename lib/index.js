...
function scanToolNames(name) {
	...
	try {
		...
		if (dir !== null) {
			...
			const hasTrace = /(?:registerTool|defineTool|tools\.register|\.register\s*\()/.test(src);
			const tools = new Set();
			for (const m of src.matchAll(/(?:defineTool|registerTool)\s*\([^)]*?name\s*:\s*['"]([^'"]+)['"]/gs)) tools.add(m[1]);
			for (const m of src.matchAll(/\.register\s*\(\s*(?:ctx\.tools\s*,\s*)?\{\s*[^}]*?name\s*:\s*['"]([^'"]+)['"]/gs)) tools.add(m[1]);
