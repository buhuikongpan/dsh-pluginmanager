...
class PluginManagerGateway extends TypertRemoteService {
	...
	async snapshot() {
		...
		for (const [entryId, state] of live) {
			...
			if (!name) continue;
			seen.add(name);
			rows.push(buildRow({ entryId, name, state, nativeNames, layers, userRows, otherBundles, bundles, deps, descriptions }));
		}
		...
		for (const row of userRows) {
			...
			rows.push(buildRow({
				...
				nativeNames, layers, userRows, otherBundles, bundles, deps, descriptions
			}));
		}
	...
	async setEnabled(input) {
		...
		const changed = input.enabled
			? (text = removeDisabledOverride(text, row.entryId), true)
			: (text = addDisabledOverride(text, row.entryId), true);
		if (!changed) return { ok: true, name, needsRestart: true, changed: false };
		backupPatch(text);
		writeText(patchPath(), text);
		return { ok: true, name, enabled: !!input.enabled, needsRestart: true, changed: true };
	}

	/**
	 * Permanently remove a user-layer plugin: its patch rows (insert block,
	 * plain overrides), its package.json entries (bundles + dependencies), and
	 * its node_modules directory. Native plugins are refused.
	 */
	async uninstall(input) {
		...
		const manifest = profileManifest();
		if (Array.isArray(manifest.dsh?.profile?.bundles) && manifest.dsh.profile.bundles.includes(name)) {
		...
		if (manifestChanged) writeJsonAtomic(manifestPath(), manifest);

		// 3. node_modules directory (covers packages not listed in dependencies)
		...
		return { ok: true, name, removed, needsRestart: true };
	}
...
function buildRow({ entryId, name, state, nativeNames, layers, userRows, otherBundles, bundles, deps, descriptions }) {
