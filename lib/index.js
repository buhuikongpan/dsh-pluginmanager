...
function buildRow({ entryId, name, state, nativeNames, layers, userRows, otherBundles, bundles, deps, descriptions, presetMap, presetInfo }) {
	...
	return {
		...
		// A plugin is a "tool plugin" only when it provides model tools. Native
		// tool-layer rows use the static table; other native rows (system/webui
		// infrastructure like the tools registry itself) must NOT be flagged even
		// though their source mentions tool APIs. User extensions are scanned for
		// real tool registrations.
		tools: layer === "tool" ? (TOOL_PROVIDED[name] ?? []) : (native ? [] : scanToolNames(name).tools),
		isToolPlugin: layer === "tool" ? true : (native ? false : scanToolNames(name).hasTrace)
	};
}
...
function scanToolNames(name) {
	...
	try {
		...
		if (dir !== null) {
			...
			// A tool plugin is one whose code defines model-callable tools — the
			// defining feature is a ToolDefinition literal, i.e. a `defineTool({...})`
			// / `registerTool({...})` call (bare after `import { defineTool }`, or
			// harness.defineTool for dynamic plugins) or registration via
			// `ctx.tools.register(...)`. Ordinary plugins use slots.register,
			// ctx.provide, ctx.on — those must NOT count as tool traces.
			const hasTrace = /(?:harness\.defineTool|harness\.registerTool|\.defineTool\s*\(|\.registerTool\s*\(|defineTool\s*\(\s*\{|registerTool\s*\(\s*\{|(?:ctx\.)?tools\.register\s*\()/.test(src);
			const tools = new Set();
