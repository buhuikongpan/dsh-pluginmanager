...
class PluginManagerGateway extends TypertRemoteService {
	...
	async uninstall(input) {
		...
		// 5. live removal: drop the running loader entry so the uninstall is
		// effective immediately. On failure the persisted changes above still
		// apply at the next boot (needsRestart fallback).
		let hot = false;
		let hotError = null;
		const entry = findLiveEntry(this.ctx.loader, name);
		if (entry !== null) {
		...
		return { ok: true, name, removed, needsRestart: !hot, hot, hotError };
	}
